...
- **`searchLocation()` に `User-Agent` を付けないこと**: Fetch 仕様の禁止ヘッダ名でブラウザが必ず落とすため、指定してもアプリ識別にはなりません（Nominatim へは Referer が送られます）。利用ポリシー上のリクエスト間隔は `LocationSearchInput` の 400ms デバウンスで守ります。
