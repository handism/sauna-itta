...
このファイルは、リポジトリ内のコードを操作する際に Antigravity へ提供するガイダンスです。
...
### ページ構成
- `/` — 地図インターフェース＋サイドバー ([page.tsx](file:///Users/mba/git/sauna-itta/src/app/page.tsx))
- `/stats` — グラフ・統計ダッシュボード＋訪問カレンダー ([page.tsx](file:///Users/mba/git/sauna-itta/src/app/stats/page.tsx))
...
`SaunaVisit` 型 ([types.ts](file:///Users/mba/git/sauna-itta/src/components/sauna-map/types.ts) 参照) には、同一サウナへの複数回訪問を記録する `history: VisitHistoryEntry[]` フィールドがあります。ID は `Date.now().toString()` で生成されます。初回ロード時に `src/data/sauna-visits.json` のシードデータが localStorage にマージされますが、既存の ID は重複追加されません。
...
- 地図・フォーム中心 of SPA のため、インタラクティブなコンポーネントにはすべて `"use client"` ディレクティブが必要です。
